...
def Entrada_Iniciar(request):
    ...
    objetinho_eprogmodel = EprogModel.objetos.using(banquinho).get(pk=1)
...
def sessao_figfig(request, pk):
        ...
        if request.POST.get('Salvar'):
            ...
            return render(request, "EprogApp/sessao_figfig.html", context)
        ...
        if request.POST.get('Sessao'):
            return redirect('url_sessao_figfig', pk=pk)
...
@login_required
def sessao_testar_figfig(request, pk):
    user = request.user
    banquinho = str(user)
    # data = {}
    objeto_procedimento = ProcedimentoModel.objetos.using(banquinho).get(pk=1)
    ...
    return render(request, "EprogApp/sessao_testar_figfig.html", context)